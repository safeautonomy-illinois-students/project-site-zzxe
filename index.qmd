...
::: {.content-text}
The system uses the Crazyflie IMU together with external pose feedback from the motion-capture system, and this information is processed through the Crazyflie state-estimation pipeline, such as its Kalman-filter-based estimator; one challenge in aggressive flight is that estimation noise, latency, or filtered-pose error can degrade tracking performance, so we plan to begin with the provided onboard estimation stack and account for these effects when designing and tuning the planner-controller pipeline.
...
If time permits, we will also explore implementing a custom low-level controller to study whether tighter integration with the onboard control stack can further improve tracking performance.
:::
